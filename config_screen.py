import pygame
from config import game_config


class ConfigScreen:
    """Configuration screen for game settings."""

    def __init__(self, width=800, height=600):
        """
        Initialize the config screen.

        Args:
            width: Screen width
            height: Screen height
        """
        self.width = width
        self.height = height

        # Colors
        self.bg_color = (20, 20, 30)
        self.title_color = (255, 255, 255)
        self.text_color = (200, 200, 200)
        self.slider_color = (100, 100, 100)
        self.slider_active_color = (255, 200, 50)
        self.button_color = (70, 70, 80)
        self.button_hover_color = (100, 100, 110)

        # Fonts
        pygame.font.init()
        self.title_font = pygame.font.Font(None, 70)
        self.label_font = pygame.font.Font(None, 50)
        self.value_font = pygame.font.Font(None, 60)
        self.button_font = pygame.font.Font(None, 50)

        # Slider state
        self.dragging = False
        self.slider_rect = pygame.Rect(250, 300, 300, 20)
        self.handle_radius = 15

        # Back button
        self.back_button = pygame.Rect(width // 2 - 100, height - 100, 200, 60)
        self.back_button_hover = False

    def _get_slider_value(self):
        """Get current maze size (1-10)."""
        return game_config.maze_size

    def _set_slider_value(self, value):
        """Set maze size (1-10)."""
        game_config.maze_size = max(1, min(10, value))

    def _get_handle_x(self):
        """Get x position of slider handle."""
        value = self._get_slider_value()
        # Map value (1-10) to slider position
        progress = (value - 1) / 9.0
        return self.slider_rect.x + int(progress * self.slider_rect.width)

    def _set_from_mouse_x(self, mouse_x):
        """Set slider value from mouse x position."""
        # Clamp to slider bounds
        x = max(self.slider_rect.x, min(mouse_x, self.slider_rect.x + self.slider_rect.width))
        # Map to value (1-10)
        progress = (x - self.slider_rect.x) / self.slider_rect.width
        value = 1 + int(progress * 9)
        self._set_slider_value(value)

    def handle_event(self, event):
        """
        Handle config screen input events.

        Args:
            event: pygame event

        Returns:
            str: Action to take ('back', None)
        """
        if event.type == pygame.MOUSEBUTTONDOWN:
            mouse_pos = pygame.mouse.get_pos()

            # Check back button
            if self.back_button.collidepoint(mouse_pos):
                game_config.save()
                return 'back'

            # Check slider
            handle_x = self._get_handle_x()
            handle_y = self.slider_rect.centery
            dist = ((mouse_pos[0] - handle_x) ** 2 + (mouse_pos[1] - handle_y) ** 2) ** 0.5
            if dist <= self.handle_radius + 5:
                self.dragging = True
            elif self.slider_rect.collidepoint(mouse_pos):
                # Click on slider bar
                self._set_from_mouse_x(mouse_pos[0])

        elif event.type == pygame.MOUSEBUTTONUP:
            if self.dragging:
                self.dragging = False
                game_config.save()

        elif event.type == pygame.MOUSEMOTION:
            mouse_pos = pygame.mouse.get_pos()

            # Update back button hover
            self.back_button_hover = self.back_button.collidepoint(mouse_pos)

            # Drag slider
            if self.dragging:
                self._set_from_mouse_x(mouse_pos[0])

        elif event.type == pygame.KEYDOWN:
            if event.key == pygame.K_ESCAPE:
                game_config.save()
                return 'back'
            elif event.key == pygame.K_LEFT:
                self._set_slider_value(self._get_slider_value() - 1)
                game_config.save()
            elif event.key == pygame.K_RIGHT:
                self._set_slider_value(self._get_slider_value() + 1)
                game_config.save()

        return None

    def render(self, screen):
        """
        Render the config screen.

        Args:
            screen: pygame surface to render to
        """
        # Background
        screen.fill(self.bg_color)

        # Title
        title_text = self.title_font.render("SETTINGS", True, self.title_color)
        title_x = self.width // 2 - title_text.get_width() // 2
        screen.blit(title_text, (title_x, 80))

        # Maze size label
        label_text = self.label_font.render("Maze Size:", True, self.text_color)
        screen.blit(label_text, (250, 220))

        # Current value
        value = self._get_slider_value()
        grid_size = 2 * value + 1
        value_text = self.value_font.render(f"{value} ({grid_size}x{grid_size})", True, self.slider_active_color)
        screen.blit(value_text, (250, 350))

        # Slider bar
        pygame.draw.rect(screen, self.slider_color, self.slider_rect, border_radius=10)

        # Slider handle
        handle_x = self._get_handle_x()
        handle_y = self.slider_rect.centery
        handle_color = self.slider_active_color if self.dragging else self.text_color
        pygame.draw.circle(screen, handle_color, (handle_x, handle_y), self.handle_radius)

        # Min/Max labels
        min_label = self.label_font.render("1", True, self.text_color)
        max_label = self.label_font.render("10", True, self.text_color)
        screen.blit(min_label, (self.slider_rect.x - 30, self.slider_rect.y - 5))
        screen.blit(max_label, (self.slider_rect.x + self.slider_rect.width + 10, self.slider_rect.y - 5))

        # Back button
        button_color = self.button_hover_color if self.back_button_hover else self.button_color
        pygame.draw.rect(screen, button_color, self.back_button, border_radius=10)
        back_text = self.button_font.render("BACK", True, self.text_color)
        text_x = self.back_button.x + (self.back_button.width - back_text.get_width()) // 2
        text_y = self.back_button.y + (self.back_button.height - back_text.get_height()) // 2
        screen.blit(back_text, (text_x, text_y))

        # Instructions
        inst_font = pygame.font.Font(None, 30)
        inst_text = inst_font.render("Use LEFT/RIGHT arrows or drag the slider", True, self.text_color)
        screen.blit(inst_text, (self.width // 2 - inst_text.get_width() // 2, 450))
